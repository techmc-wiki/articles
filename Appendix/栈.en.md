---

slug: stack
title: 栈的概念
title-en: "The Concept of the Stack"
author: Twisuki
co-authors:
- Arcadi4
- BFladderbeanawa
- hotpad100c
- Molforte
date: '2025-01-21T20:09:13+08:00'
lastmod: '2026-04-05T22:16:03+08:00'
index: 1
is-advanced: false
banner:
  src: img/banner.png
  alt: Graduate Text in Minecraft
---


This chapter introduces the concepts of stacks and call stacks, which will help readers better understand block updates.

## Understanding Stacks

> A **stack** is a **first-in-last-out (FILO, First In Last Out)** data structure, characterized by the fact that elements can only be operated on at one end of the structure.

### First In Last Out

Imagine a **Pringles can**:

- Chips can only be placed in from the top, and can only be taken out from the top.
- Therefore, **the chip placed at the bottom first will be the last one that can be taken out**.

This is the concept of "first in last out".

### Stack

A stack is like a Pringles can.
In data structure terminology:

- "Putting chips into the can" corresponds to **push**;
- "Taking chips from the top" corresponds to **pop**.

Additionally:

- The **top of the can** is equivalent to the **top of the stack**
- The **bottom of the can** is equivalent to the **bottom of the stack**

This is the concept of a stack.

## The Concept of Call Stack

> A **call stack** is a mechanism that programs use at runtime to manage function calls. It relies on the "first-in-last-out" rule of stacks to ensure functions can return correctly.

## Call Stack

Imagine you're making phone calls to ask a question:

1. You call your friend A to ask a question.
2. A doesn't know the answer, so they call their friend B to ask.
3. B isn't sure either, so they call their friend C to ask.

This time, C finds the answer and tells B first;  
B then tells the answer to A;  
Finally, A tells you the answer.

The order here is **the behavior of a call stack**:

- Each phone call pushes a new "call task" onto the "call stack".
- When C is searching for the answer, **the task at the top of the stack** is "C searching for the answer".
- After C finds the answer, **the last called task completes first**, then returns layer by layer to the callers (B → A → you).
- Your initial phone call was the first task pushed onto the call stack, so it sits at the **bottom of the stack**.

In a program:

- "Making a phone call" is equivalent to **calling a function**;
- "Getting an answer and passing it back" is equivalent to **a function returning a result**.

This way, when functions are nested, the program can ensure each function completes and correctly returns to its calling point.
