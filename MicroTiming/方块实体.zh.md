...
flowchart LR
    Start([开始])
    A["<pre>Cooldown -1</pre><br/>记录当前时间"]
    B{"无需冷却?"}
    C["cooldown设为0"]
    D{"无需冷却且未上锁?"}
    E["具体的逻辑"]
...
flowchart TD
    ...
    E["执行拉取操作"]
    F{"输出和拉取操作<br/>有一方成功了?"}
    G["<pre>Cooldown = 8</pre><br/>更新、同步此方块实体"]
    End([End])
...
先看_**输出**_...
_**输出**_输出操作由 `insert()` 方法完成，它的作用是将物品从漏斗传输到目标容器，并返回一个布尔值来表示是否成功传输。如果传输成功，则漏斗进入冷却状态。
...
flowchart LR
    Start([开始])
    A{"这个容器有<br/>侧向槽位吗"}
    B["拿到在该方向可插入的所有槽位，<br/>逐槽尝试 transfer（输出）方法"]
    C["拿到目标容器的所有槽位，<br/>逐槽尝试 transfer（输出）方法"]
    End([结束])
...
到此，输出的部分已经完成，接下来，我们来看看_**拉取**_的部分。
拉取物品的逻辑与输出类似，但方向相反：
...
flowchart LR
    Start([开始])
    A{"上方有容器"}
    B["拿到容器的所有槽位，<br/>逐槽尝试 transfer（输出）方法"]
    C{"上方1格内<br/>未被阻挡"}
    D["尝试从掉落物中取出物品"]
    End([结束])
...
flowchart TD
    ...
    G["目标漏斗给予目标漏斗<br/>8gt冷却"]
    H["目标漏斗给予目标漏斗<br/>7gt冷却"]
