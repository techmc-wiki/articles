...
slug: scheduled-ticks
title: 计划刻与计划刻元件
title-en: "Scheduled Ticks and Scheduled Tick Components"
author: hotpad100c
co-authors:
- Arcadi4
- BFladderbeanawa
...
This section will explain how Scheduled Ticks and Scheduled Tick Components operate.
...
- Timing analysis of 4gt Observer high-frequency
- In-depth analysis of Comparator's Scheduled Tick execution logic
...
In Minecraft, we often see many redstone components that do not change immediately after being triggered, such as repeaters and comparators. These components always change after a delay following their trigger.

Let's look at an example from daily life. In the morning, you receive an email, so you plan to handle it in the afternoon and set an alarm. When afternoon comes, the alarm rings, you remember you need to handle the email, so you open your mailbox and start processing it.

It's important to note: **This "alarm" has no "text annotation". It only reminds you on your phone, at the correct time, that someone "has something to do". As for what specifically needs to be done, the alarm doesn't care.** This alarm contains and only contains these elements:
...
A Scheduled Tick is such an "alarm". When a redstone component is triggered, it adds a Scheduled Tick for itself. When the Scheduled Tick executes, it's like the alarm ringing, and the redstone component changes.
...
Like the alarm in the example above, **a Scheduled Tick only contains these elements**, which we call the **information structure of a Scheduled Tick**:
...
- Priority `priority`: how prioritized the Scheduled Tick is
...
**Position** and **block type** are easy-to-understand attributes. After all, a block's own Scheduled Tick cannot be executed by other blocks ~~(unless you happen to push it away or break it and replace it with another block before the block executes its Scheduled Tick)~~, nor can it run around in the world.

**Execution time** refers to the execution time of the Scheduled Tick in macro timing, that is, in which gt the Scheduled Tick should execute. For example, when a 1-delay repeater is triggered, it adds a Scheduled Tick for 2gt later, and after 2gt, that Scheduled Tick will be executed. What we commonly call "the delay of a Scheduled Tick component" refers to how many gt later the Scheduled Tick executes.

**Sub-order** refers to the order in which Scheduled Ticks are added within the same time. For example, within the same gt, repeater A is triggered first, and repeater B is triggered later. Then, in the sub-sequence, repeater A comes before repeater B.

**Priority** refers to the priority level of the Scheduled Tick. Priority is an integer from -3 to 3[^2], where smaller values mean higher priority. That is, within the same gt, Scheduled Ticks with priority -3 always execute before those with priority -2, -1, or 0.
...
Similarly, just like the alarm above: **A Scheduled Tick is just an "alarm reminding the block that something needs to be done", it doesn't care what the block actually needs to do. All behaviors when executing a Scheduled Tick are controlled by the block itself, and "the behavior of executing a Scheduled Tick" is not in "the information structure of a Scheduled Tick".**

When we say "a certain component already has a Scheduled Tick", we mean that at the current position, there exists a Scheduled Tick with the same block type as the current block that has not yet been executed.[^3]

[^3]: Due to the need for explanation in later sections, the basic section only provides a simple description. Specific content involves the maintenance of Scheduled Tick queues, see the advanced section.

### Execution Order of Scheduled Ticks

In the previous two articles, we have learned about macro timing analysis and become familiar with micro timing. We know that **macro timing always takes precedence over micro timing**, and the same applies to Scheduled Ticks. For Scheduled Ticks with different execution times, those with earlier execution times always execute first. For Scheduled Ticks with the same execution time, those with higher priority always execute first. For Scheduled Ticks with the same priority, those with smaller sub-sequence values, i.e., those added earlier, always execute first.
...
This is like comparing numbers, where macro timing is the hundreds place, priority is the tens place, and addition order is the ones place.
...
1. If in different gt, you press the comparator's button first, then press the repeater's button, which note block will sound first?
2. If in the same gt, you press the comparator's button first, then press the repeater's button, which note block will light up first?

Answers:

1. The comparator lights up first. Because in macro timing, the comparator lights up first, and then after a certain number of gt, the repeater lights up.
2. The repeater lights up first. Because in terms of Scheduled Ticks, although the comparator adds its Scheduled Tick before the repeater, the repeater's priority is -1, which is more prioritized than the comparator's priority 0, and Scheduled Tick priority > Scheduled Tick addition order, so the repeater lights up first.
...
In [Tick and Inter-Tick Timing](01-刻与刻间时序.md#12-中继器比较器初步), we have already had a preliminary understanding of repeaters and comparators. Now, let's delve into the Scheduled Tick behavior of repeaters.

**If a repeater is locked, it will not add a Scheduled Tick, nor will it change any state when executing a Scheduled Tick.** If a repeater is not locked, it has the following behaviors:
...
From a behavioral perspective, for instance, if you give a 2-delay repeater a signal with `duration <= 4gt`[^5], the repeater's behavior is as follows:
...
This example contains all the Scheduled Tick behaviors of a repeater.
...
The **output power level** here refers to the output power level calculated by the comparator _(specific calculation method is described later)_. And this **update** includes two things: **updating the power level** and **sending updates**. Moreover, for comparators, from an observational perspective, there may be cases where the output power level does not change, but in reality, the comparator still "updates" its own output power level.

Comparators also have relatively more complex update behavior—
...
- First, this entire section is based on the premise that the comparator **actually executed a Scheduled Tick**—
...
These behaviors may be applied in some special wiring. If readers find this section difficult to understand, they can temporarily only remember the most common uses of comparators—comparing signal sizes and determining whether to output, detecting container capacity, and subtract mode, or simply refer to a table.
...
If the signal input is redstone dust or another comparator, it inherits the input power.

If it's a container, please refer to this section [Comparator Signal Strength Calculation](BlockUpdate/更新概念与不同类型的更新?id=_152-比较器信号强度计算).

In particular, when there are both container signals and redstone signal inputs, the comparator will prioritize different signal inputs for calculation based on the situation. This phenomenon is also called container shielding.
...
Case (barrels all have no items):
...
At this point, we can now analyze the case from the previous chapter
...
**Usually not.** Let's analyze this briefly:

Both observers and redstone torches have a priority of **0**. And when an observer lights up, it **first sends a PP update**, then **adds a Scheduled Tick**, and finally **sends an NC update**. Redstone torches **receive NC updates**. The **order of behaviors within executing a Scheduled Tick** and **what updates a component receives** fundamentally affect the following analysis. So assuming an observer is facing a solid block that a redstone torch is attached to, its Scheduled Tick order is as follows:
...
**However**, at this point we can discover a problem: if I place two observers, one observer is used to **"trigger" the redstone torch to add a Scheduled Tick**, and another is used to **"continue" the redstone signal to turn off the torch**, then can the redstone torch light up? Yes. A simple structure is shown in the figure below:
...
To improve the reader's fluency when reading the **Scheduled Tick execution process**, we will first explain all important concepts and methods involved, and finally explain the execution logic of Scheduled Ticks. When reading, you can first get a preliminary understanding of these concepts and methods, and then refer back when encountering difficulties in understanding the Scheduled Tick execution logic.
...
Attempts to add the **hashed Scheduled Tick** to the **hash set**. If the addition is successful (i.e., the Scheduled Tick does not exist in the set), it simultaneously adds the **Scheduled Tick object** of that Scheduled Tick to the **priority queue**.
...
The priority queue's sorting is based on [**Execution Time Comparison**](#execution-time-comparison), meaning it will consider `triggerTick`, `priority`, and `subTickOrder` simultaneously.
...
The World-level Scheduler manages the "checking" and execution of Scheduled Ticks. During the Scheduled Tick execution phase, it coordinates and manages all Chunk-level Schedulers, and polls Scheduled Ticks in a certain order according to certain logic, placing them into the executable Scheduled Tick list, and finally executes them all together.

See [Executing Scheduled Ticks](#executing-scheduled-ticks).

### Checking if a Scheduled Tick Exists

This method is what we commonly call "a certain component does not have a Scheduled Tick" or "a certain component does not have a Scheduled Tick for itself at the current position". Obviously, it describes two core parameters: "a certain component"—block type `type`, "itself" "current position"—coordinates `pos`.
...
It receives two parameters: block type and coordinates, first copies the queue of Scheduled Ticks that will execute in the current gt, then checks whether there is a Scheduled Tick with the same block type and coordinates in this queue.
...
1. Scan all Chunk-level Schedulers (responsible for managing Scheduled Ticks within a single chunk). Only select those that:
   ...
   - The chunk corresponding to that Scheduled Tick itself meets loading requirements.
...
4. **Attempt to continuously extract more Scheduled Ticks:** Next, the game will try to continuously poll as many subsequent Scheduled Ticks as possible from the current Chunk-level Scheduler (as long as specific conditions are met):
   ...
   6. **Decide whether to poll (core condition):**
      - If the current Chunk-level Scheduler indeed still has Scheduled Ticks (successfully accessed its current head Scheduled Tick),

      - And this Scheduled Tick also needs to execute in the current game tick (gt),

      - And using the **Simple Comparison** rule to judge, this Scheduled Tick of the current Chunk-level Scheduler has an earlier execution time than the head Scheduled Tick of Chunk-level Scheduler 2,
...
6. Loop through the above steps (steps 1 to 5) until the Scheduled Tick limit is reached or the scheduler queue is polled empty.

In summary, the above steps can be described as follows:

The game first filters out all Chunk-level Schedulers that have Scheduled Ticks to execute in the current gt and whose chunks are loaded, and queues them according to the Simple Comparison order of their most prioritized Scheduled Tick (head Scheduled Tick). Then, it polls a Chunk-level Scheduler from the queue head, adds its most prioritized Scheduled Tick to the ready-to-execute queue, and attempts to "continuously" extract more tasks from this scheduler that also execute in the current gt and are more prioritized than the most prioritized Scheduled Tick of the next scheduler in the queue to add to the ready-to-execute queue together (the purpose is to improve efficiency). If continuous extraction is interrupted (such as the next Scheduled Tick is not prioritized enough, or the execution limit is reached), and this chunk still has Scheduled Ticks for the current gt that have not been added and the limit has not been exceeded, the current scheduler is put back into the scheduler queue according to the priority order of the head Scheduled Tick, so that it can continue processing its remaining tasks later.
