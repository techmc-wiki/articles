...
if (bl) {
    this.chunkToNextSaveTimeMs.put(l, n + 10000L);  // Set 10-second cooldown after successful save
}
```
...
Key sequence in `ThreadedAnvilChunkStorage.save(Chunk)` (1.20.1 Yarn `ThreadedAnvilChunkStorage.java`:816-840):

```java
if (!chunk.needsSaving()) { return false; }
chunk.setNeedsSaving(false);                                             // Clear dirty flag
NbtCompound nbtCompound = ChunkSerializer.serialize(this.world, chunk);   // Main thread synchronous serialization
this.setNbt(chunkPos, nbtCompound);                                      // Submit to StorageIoWorker (background worker thread responsible for async disk writes)
```
...
Auto-save triggered every 6000 ticks (1.20.1 Yarn `MinecraftServer.tick()`):
...
The non-flush branch of `ThreadedAnvilChunkStorage.save(false)` (1.20.1 Yarn `ThreadedAnvilChunkStorage.java`:469-471):

```java
this.chunkHolders.values().forEach(this::save);    // Reuses save(ChunkHolder), subject to cooldown
```
...
When `WorldChunk.setBlockState()` executes on the main thread (1.20.1 Yarn):
...
Dispenser is activated by redstone; 4 ticks later `ServerWorld`'s `blockTickScheduler.tick()` executes the scheduled tick and calls `DispenserBlock.scheduledTick()`, firing the dispenser.
...
`StorageIoWorker` background thread continues writing remaining NBT in `results`.
